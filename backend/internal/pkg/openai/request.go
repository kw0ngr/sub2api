...
// 保留尾随空格，不能放入会 TrimSpace 的 prefix 列表，否则会退化成裸 "codex"。
const codexOfficialClientFamilyPrefix = "codex "

// codexOfficialClientOriginators 是 Codex 官方客户端家族 originator 精确集合。
...
// 精确集合匹配 + `Codex ` 家族前缀，避免 codex_* 伪造值绕过。
...
func matchCodexClientHeaderPrefixes(value string, prefixes []string) bool {
	for _, prefix := range prefixes {
		...
		// 优先前缀匹配；若 UA 被网关拼接为复合字符串时，退化为包含匹配。
...
// User-Agent. ChatGPT Codex upstream rejects mismatched originator/UA pairs.
