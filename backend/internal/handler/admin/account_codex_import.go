...
// CodexSessionImportRequest imports Codex/OpenAI OAuth session material into
// OpenAI OAuth accounts without touching fork-only account management features.
...
// ImportCodexSession imports one or more Codex session JSON blobs or raw
// access tokens as OpenAI OAuth accounts.
...
func (h *AccountHandler) importCodexSessions(ctx context.Context, req CodexSessionImportRequest, entries []codexImportEntry) (CodexSessionImportResult, error) {
	...
	for _, entry := range entries {
		...
		if err != nil {
			result.addCodexFailure(entry.Index, "", err)
			continue
		}
		...
		if expiryErr != nil {
			result.addCodexFailure(entry.Index, accountName, expiryErr)
			continue
		}
		...
		if existing != nil {
			...
			if updateErr != nil {
				result.addCodexFailure(entry.Index, accountName, updateErr)
				continue
			}
		...
		if createErr != nil {
			result.addCodexFailure(entry.Index, accountName, createErr)
			continue
		}
		if account != nil {
...
func (r *CodexSessionImportResult) addCodexFailure(index int, name string, err error) {
	message := err.Error()
	r.Failed++
	r.Items = append(r.Items, CodexSessionImportItem{
		Index:   index,
		Name:    name,
		Action:  "failed",
		Message: message,
	})
	r.Errors = append(r.Errors, CodexSessionImportMessage{
		Index:   index,
		Name:    name,
		Message: message,
	})
...
func looksLikeCodexJSON(content string) bool {
	if content == "" {
		return false
	}
	switch content[0] {
