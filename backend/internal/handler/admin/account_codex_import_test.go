...
func TestSanitizeCodexImportCredentialExtrasProtectsTokenIdentityFields(t *testing.T) {
	extras := sanitizeCodexImportCredentialExtras(map[string]any{
		"access_token":  "attacker-access",
		"refresh_token": "attacker-refresh",
		"client_id":     "attacker-client",
		"model_mapping": map[string]any{"from": "gpt-5.4", "to": "gpt-5.5"},
		"custom_flag":   true,
	})

	if _, ok := extras["access_token"]; ok {
		t.Fatalf("access_token should be protected")
	}
	if _, ok := extras["refresh_token"]; ok {
		t.Fatalf("refresh_token should be protected")
	}
	if _, ok := extras["client_id"]; ok {
		t.Fatalf("client_id should be protected")
	}
	if _, ok := extras["model_mapping"]; !ok {
		t.Fatalf("model_mapping should be preserved")
	}
	if extras["custom_flag"] != true {
		t.Fatalf("custom_flag = %v, want true", extras["custom_flag"])
	}
}
...
func TestBuildCodexImportIdentityKeysAccessOnlyUsesTokenFingerprint(t *testing.T) {
	keys := buildCodexImportIdentityKeys("shared-account", "user-1", "same@example.com", "access-token", "")

	if len(keys) != 1 || !strings.HasPrefix(keys[0], "access:") {
		t.Fatalf("access-only import should use only access fingerprint, got %v", keys)
	}
}

func TestCodexAccountIndexSkipsSharedAccountIDWhenUserDiffers(t *testing.T) {
	...
	keys := buildCodexStoredIdentityKeys("team-account", "user-b", "", "")

	account, matchedKey := index.Find(keys, "user-b")

	if account != nil || matchedKey != "" {
		t.Fatalf("expected shared account id conflict to be skipped, got account=%v key=%q", account, matchedKey)
	}
}
