...
// GetProjectStats handles getting project attribution usage statistics.
...
func (h *DashboardHandler) GetProjectStats(c *gin.Context) {
	...
	stats, err := h.dashboardService.GetProjectStatsWithFilters(
		c.Request.Context(),
		startTime,
		endTime,
		userID,
	...
	if err != nil {
		response.Error(c, 500, "Failed to get project statistics")
		return
	}
...
func (h *DashboardHandler) GetUserBreakdown(c *gin.Context) {
	...
	if requestTypeStr := strings.TrimSpace(c.Query("request_type")); requestTypeStr != "" {
		parsed, err := service.ParseUsageRequestType(requestTypeStr)
		if err != nil {
			response.BadRequest(c, "Invalid request_type, use sync/stream/ws_v2")
			return
		}
		rtVal := int16(parsed)
