...
func openAIChannelForwardModelForScheduler(requestedModel string, channelMapping service.ChannelMappingResult) string {
...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	legacyCompact := service.IsOpenAIResponsesCompactPathForTest(c)
	...
	for {
		...
		selection, scheduleDecision, err := h.gatewayService.SelectAccountWithSchedulerForPlatformAndCapability(
			requestCtx,
			gatewayPlatform,
			apiKey.GroupID,
			previousResponseID,
			sessionHash,
			openAIChannelForwardModelForScheduler(reqModel, channelMapping),
			failedAccountIDs,
			service.OpenAIUpstreamTransportAny,
			openAIResponsesRequiredCapabilityForRequest(nativeV2 || legacyCompact, gatewayPlatform),
			legacyCompact,
		)
		if err != nil {
		...
		if acquireFailoverErr != nil {
			...
			reqLog.Warn("openai.account_slot_local_failover_switching", zap.Int64("account_id", account.ID), zap.Int("switch_count", switchCount), zap.Int("max_switches", maxAccountSwitches))
			continue
		}
		if !acquired {
			return
...
func openAIResponsesRequiredCapabilityForRequest(needsResponses bool, platform string) service.OpenAIEndpointCapability {
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		if acquireFailoverErr != nil {
			...
			switchCount++
			setDebugTraceAccountSwitchCount(c, switchCount)
			reqLog.Warn("openai_messages.account_slot_local_failover_switching", zap.Int64("account_id", account.ID), zap.Int("switch_count", switchCount), zap.Int("max_switches", maxAccountSwitches))
			continue
		}
		if !acquired {
			return
...
func (h *OpenAIGatewayHandler) acquireResponsesAccountSlot(
	...
	streamStarted *bool,
	reqLog *zap.Logger,
) (func(), bool, *service.UpstreamFailoverError) {
	if selection == nil || selection.Account == nil {
	...
	if waitErr != nil {
		...
		return nil, false, &service.UpstreamFailoverError{
			StatusCode:   http.StatusTooManyRequests,
			ResponseBody: []byte(`{"error":{"type":"rate_limit_error","message":"account wait queue full"}}`),
		}
	}
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	project := ExtractUsageProject(c, firstMessage)
	inboundEndpoint := GetInboundEndpoint(c)
	...
	ensureUserSlotHeld := func() bool {
		...
		releaseFunc, acquired, err := h.concurrencyHelper.TryAcquireUserSlot(ctx, subject.UserID, subject.Concurrency)
		if err != nil {
		...
		currentUserRelease = wrapReleaseOnDone(ctx, releaseFunc)
		return true
	}
	...
	for {
		selection, scheduleDecision, err := h.gatewayService.SelectAccountWithSchedulerForPlatform(
			ctx,
			resolveOpenAICompatibleGatewayPlatform(apiKey),
			apiKey.GroupID,
			previousResponseID,
			sessionHash,
			openAIChannelForwardModelForScheduler(reqModel, channelMappingWS),
			failedAccountIDs,
		...
		if !selection.Acquired {
			...
			fastReleaseFunc, fastAcquired, acquireErr := h.concurrencyHelper.TryAcquireAccountSlot(ctx, account.ID, selection.WaitPlan.MaxConcurrency)
			if acquireErr != nil {
				reqLog.Warn("openai.websocket_account_slot_acquire_failed", zap.Int64("account_id", account.ID), zap.Error(acquireErr))
		...
		turnFirstMessage := wsFirstMessage
		if previousResponseID != "" && !scheduleDecision.StickyPreviousHit &&
			!service.ValidateFunctionCallOutputContextBytes(turnFirstMessage).HasFunctionCallOutput {
			turnFirstMessage = service.RemovePreviousResponseIDFromBody(turnFirstMessage)
			reqLog.Debug("openai.websocket_previous_response_id_stripped_cross_group",
				zap.Int64("account_id", account.ID),
				zap.String("schedule_layer", scheduleDecision.Layer),
			)
		}
		requestPayloadHash := service.HashUsageRequestPayload(turnFirstMessage)
		upstreamEndpoint := GetUpstreamEndpoint(c, account.Platform)
			...
			AfterTurn: func(turn int, result *service.OpenAIForwardResult, turnErr error) {
				...
				h.submitOpenAIUsageRecordTask(result, func(taskCtx context.Context) {
					if err := h.gatewayService.RecordUsage(taskCtx, &service.OpenAIRecordUsageInput{
						...
						APIKeyService: h.apiKeyService, ChannelUsageFields: channelMappingWS.ToUsageFields(reqModel, result.UpstreamModel),
					}); err != nil {
						reqLog.Error("openai.websocket_record_usage_failed", zap.Int64("account_id", account.ID), zap.String("request_id", result.RequestID), zap.Error(err))
					}
				})
			},
		}
		...
		if err := h.gatewayService.ProxyResponsesWebSocketFromClient(ctx, c, wsConn, account, token, turnFirstMessage, hooks); err != nil {
			...
			h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, false, nil)
			closeStatus, closeReason := summarizeWSCloseErrorForLog(err)
			reqLog.Warn("openai.websocket_proxy_failed", zap.Int64("account_id", account.ID), zap.Error(err), zap.String("close_status", closeStatus), zap.String("close_reason", closeReason))
...
// ensureForwardErrorResponse appends an SSE terminal error when headers were already flushed.
