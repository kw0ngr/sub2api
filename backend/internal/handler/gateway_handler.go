...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	// Get available models from account configurations for the effective routing platform.
...
// gatewayForwardErrorAlreadyCommunicated reports whether Forward already wrote
// a complete non-SSE error response before returning an error. SSE streams may
// only have emitted keepalives or partial data, so they still need the fallback
...
func (h *GatewayHandler) CountTokens(c *gin.Context) {
	...
	if err != nil {
		...
		cls := classifyNoAccountErrorFromGin(c, noAccountDiagnosisRequest{
			Diagnoser:    h.gatewayService,
			APIKey:       apiKey,
			RoutingModel: parsedReq.Model,
			DisplayModel: parsedReq.Model,
...
// 这类请求用于 Claude Code 验证 API 连通性；CCSwitch 健康检查也可能使用流式请求。
