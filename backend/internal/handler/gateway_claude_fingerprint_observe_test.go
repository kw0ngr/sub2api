...
func TestGatewayHandlerObserveClaudeCodeFingerprintCandidateIgnoresApplySwitch(t *testing.T) {
	...
	require.Equal(t, "application/json", profile.Accept)
	require.Equal(t, "2023-06-01", profile.AnthropicVersion)
	require.Equal(t, "cli", profile.XApp)
	require.Equal(t, "true", profile.DirectBrowserAccess)
	require.Equal(t, "0.71.0", profile.StainlessPackageVersion)
	require.Equal(t, "Darwin", profile.StainlessOS)
	require.Equal(t, "0", profile.StainlessRetryCount)
	require.Equal(t, "600", profile.StainlessTimeout)
	require.GreaterOrEqual(t, profile.CompletenessScore, 90)
