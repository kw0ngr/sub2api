...
func (r *channelRepository) ListModelPricing(ctx context.Context, channelID int64) ([]service.ChannelModelPricing, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_read_price, reasoning_effort_multipliers, image_output_price, per_request_price, created_at, updated_at
...
func (r *channelRepository) UpdateModelPricing(ctx context.Context, pricing *service.ChannelModelPricing) error {
	...
	result, err := r.db.ExecContext(ctx,
		`UPDATE channel_model_pricing
		 SET models = $1, billing_mode = $2, input_price = $3, output_price = $4, cache_write_price = $5, cache_read_price = $6, reasoning_effort_multipliers = $7, image_output_price = $8, per_request_price = $9, platform = $10, updated_at = NOW()
		 WHERE id = $11`,
		modelsJSON, billingMode, pricing.InputPrice, pricing.OutputPrice, pricing.CacheWritePrice, pricing.CacheReadPrice,
		reasoningJSON, pricing.ImageOutputPrice, pricing.PerRequestPrice, pricing.Platform, pricing.ID,
	)
	if err != nil {
...
func scanModelPricingRows(rows *sql.Rows) ([]service.ChannelModelPricing, []int64, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&reasoningJSON, &p.ImageOutputPrice, &p.PerRequestPrice, &p.CreatedAt, &p.UpdatedAt,
		); err != nil {
...
func createModelPricingExec(ctx context.Context, exec dbExec, pricing *service.ChannelModelPricing) error {
	...
	err = exec.QueryRowContext(ctx,
		`INSERT INTO channel_model_pricing (channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_read_price, reasoning_effort_multipliers, image_output_price, per_request_price)
		...
		reasoningJSON, pricing.ImageOutputPrice, pricing.PerRequestPrice,
