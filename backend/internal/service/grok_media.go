...
func (s *OpenAIGatewayService) ForwardGrokMedia(
	...
	// cli-chat-proxy requires Grok CLI identity headers (else 426 outdated CLI).
	applyGrokCLIClientHeaders(upstreamReq.Header, account)
	if endpoint.RequiresRequestBody() {
	...
	if err != nil {
		return nil, s.handleOpenAIUpstreamTransportError(ctx, c, account, err, false, requestInfo.Model)
	}
