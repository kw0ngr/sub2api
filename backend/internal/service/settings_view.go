...
type SystemSettings struct {
	...
	OpenAICyberSafetyRetryEnabled      bool // 是否屏蔽 OpenAI 网络安全策略响应并换账号重试（默认 false）
	OpenAIGPT56SolDefaultMaxReasoning  bool // 是否在 gpt-5.6-sol 缺省 reasoning 时默认注入 max（默认 false）
...
// RateLimit429CooldownSettings controls short fallback cooldowns when a 429
// response has no reliable reset header.
