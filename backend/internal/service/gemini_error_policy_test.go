...
func TestHandleGeminiUpstreamError_GoogleOneCapacityExhaustedUsesTierCooldown(t *testing.T) {
	...
	require.WithinDuration(t, before.Add(5*time.Minute), *repo.lastRateLimitResetAt, after.Sub(before)+2*time.Second)
	require.True(t, repo.lastRateLimitResetAt.After(before))
	require.True(t, repo.lastRateLimitResetAt.Before(after.Add(5*time.Minute).Add(2*time.Second)))
}
