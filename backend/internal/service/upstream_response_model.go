...
type upstreamResponseModelObserver struct {
	first               string
	terminal            string
	firstServiceTier    string
	terminalServiceTier string
	conflict            bool
}
...
func (o *upstreamResponseModelObserver) ObserveOpenAI(payload []byte, eventType string) {
	...
	tier := firstValidTrimmedGJSONModel(payload, "response.service_tier", "service_tier")
...
func (o *upstreamResponseModelObserver) ObserveServiceTier(raw string, terminal bool) {
	serviceTier := normalizeOpenAIServiceTier(raw)
	if serviceTier == nil {
		return
	}
	if terminal {
		o.terminalServiceTier = *serviceTier
		return
	}
	if o.firstServiceTier == "" {
		o.firstServiceTier = *serviceTier
	}
}
...
func (o *upstreamResponseModelObserver) ServiceTier() *string {
	...
	value := o.terminalServiceTier
	if value == "" {
		value = o.firstServiceTier
	}
	if value == "" {
		return nil
	}
...
func ensureUpstreamResponseModelObservation(c *gin.Context) *upstreamResponseModelObserver {
	if observer := upstreamResponseModelObserverFromContext(c); observer != nil {
		return observer
	}
	return beginUpstreamResponseModelObservation(c)
}
