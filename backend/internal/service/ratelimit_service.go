...
genericRuntimePath:
	...
	if statusCode == http.StatusTooManyRequests && account.Platform == PlatformAnthropic {
		...
		if hasAnthropicExhaustedWindow(headers) {
	...
	case 401:
		...
		if account.Type == AccountTypeOAuth && account.Platform != PlatformAntigravity {
			...
			// 不写回 account.Credentials/expires_at：这里拿到的是请求开始时的账号快照，
			// 整列覆盖 JSONB 可能把其他 worker 刚刷新的 refresh_token 回滚成旧值。
...
func selectAnthropicFableWindowReset(headers http.Header, now time.Time) *time.Time {
	if !isAnthropicWindowRejected(headers, "7d_oi") && !isAnthropicWindowExceeded(headers, "7d_oi") {
		return nil
	}
	resetAt, ok := parseAnthropicResetTimestamp(headers.Get("anthropic-ratelimit-unified-7d_oi-reset"), now, 8*24*time.Hour)
	if !ok {
		resetAt, ok = parseAnthropicResetTimestamp(headers.Get("anthropic-ratelimit-unified-reset"), now, 8*24*time.Hour)
	}
	if !ok {
		return nil
	}
...
func (s *RateLimitService) persistAnthropicFableWindowLimit(ctx context.Context, account *Account, headers http.Header) bool {
	...
	resetAt := selectAnthropicFableWindowReset(headers, time.Now())
	if resetAt == nil {
		return false
	}
...
func parseOpenAIRateLimitPlanType(body []byte) string {
	...
	if errCode == "" {
		errCode, _ = errObj["type"].(string)
		errCode = strings.ToLower(strings.TrimSpace(errCode))
	}
	if errCode != "usage_limit_reached" && errCode != "rate_limit_exceeded" {
		return ""
	}
...
func (s *RateLimitService) HandleOpenAIImageRateLimit(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte) bool {
	...
	if err := s.accountRepo.SetModelRateLimit(ctx, account.ID, openAIImageGenerationRateLimitKey, resetAt); err != nil {
		slog.Warn("openai_image_rate_limit_set_model_rate_limit_failed", "account_id", account.ID, "scope", openAIImageGenerationRateLimitKey, "reason", openAIImageRateLimitReason, "error", err)
		return true
	}
	slog.Info("openai_image_rate_limited", "account_id", account.ID, "scope", openAIImageGenerationRateLimitKey, "reason", openAIImageRateLimitReason, "reset_at", resetAt, "reset_in", time.Until(resetAt).Truncate(time.Second))
	return true
}
...
func (s *RateLimitService) HandleUpstreamModelNotFound(ctx context.Context, account *Account, requestedModel string, statusCode int, responseBody []byte) bool {
	...
	if account.Platform == PlatformAntigravity {
		modelKey = strings.TrimSpace(resolveFinalAntigravityModelKey(ctx, account, modelKey))
	} else {
...
func (s *RateLimitService) tryTempUnschedulable(ctx context.Context, account *Account, statusCode int, responseBody []byte, requestedModel ...string) bool {
	...
	for idx, rule := range rules {
		...
		if s.triggerTempUnschedulable(ctx, account, rule, idx, statusCode, matchedKeyword, responseBody, firstRequestedModel(requestedModel)) {
			return true
		}
	}
...
func (s *RateLimitService) triggerTempUnschedulable(ctx context.Context, account *Account, rule TempUnschedulableRule, ruleIndex int, statusCode int, matchedKeyword string, responseBody []byte, requestedModel ...string) bool {
	...
	if modelKey != "" && statusCode != http.StatusUnauthorized {
		if err := s.accountRepo.SetModelRateLimit(ctx, account.ID, modelKey, until); err != nil {
			...
			// 当前请求仍应切换账号；持久化失败时也不能把模型故障扩大成整账号封禁。
			return true
		}
