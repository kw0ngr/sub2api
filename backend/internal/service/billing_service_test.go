...
func TestGetFallbackPricing_FamilyMatching(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "claude opus 4.8", model: "claude-opus-4.8-20260529", expectedInput: 5e-6},
		...
		{name: "openai gpt5.1", model: "gpt-5.1", expectedInput: 2.5e-6},
		{name: "openai gpt5.6 sol", model: "gpt-5.6-sol", expectedInput: 2.5e-6},
		{name: "openai gpt5.6 terra", model: "gpt-5.6-terra", expectedInput: 2.5e-6},
		{name: "openai gpt5.6 luna", model: "gpt-5.6-luna", expectedInput: 2.5e-6},
...
func TestGetModelPricingWithChannel_DoesNotMutateFallbackPricing(t *testing.T) {
	...
	original, err := svc.GetModelPricing("claude-sonnet-4")
	require.NoError(t, err)
	require.Greater(t, math.Abs(original.InputPricePerToken-99e-6), 1e-12)
}
