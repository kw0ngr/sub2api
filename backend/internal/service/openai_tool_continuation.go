...
// AnalyzeToolContinuationSignals extracts Codex tool output, call context and reference signals.
// FunctionCallOutput field names are retained for compatibility with existing callers.
...
// HasToolCallContext 判断 input 是否包含带 call_id 的 Codex 工具调用上下文。
