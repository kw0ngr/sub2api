...
func ClassifyAPIKeyStatusAction(account *Account, statusCode int, responseBody []byte) APIKeyStatusAction {
	...
	if statusCode == http.StatusBadRequest && (code == "unsupported_parameter" || code == "unknown_parameter") {
		return APIKeyStatusActionIgnore
	}
...
func isClientRequestParameterValidationError(msg string) bool {
	...
	if strings.Contains(msg, "call_id") &&
		containsAny(msg, "string too long", "maximum length", "string_above_max_length") {
		return true
	}
	return false
}
