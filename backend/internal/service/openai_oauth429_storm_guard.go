...
func (s *OpenAIGatewayService) RecordOpenAIOAuth429(account *Account, statusCode int) {
	if s == nil || statusCode != http.StatusTooManyRequests || !isOpenAIOAuthAccount(account) {
		return
	}
	now := time.Now()
