...
type OpenAIWSIngressHooks struct {
	// InitialRequestModel is the pre-channel-mapping model from the first
	// inbound frame. It is only for usage metadata reasoning-effort inference;
	// it must not affect upstream routing or billing model selection.
...
// Replay ownership invariant: json.RawMessage bodies are immutable after entering
// replay state. Owners may share body bytes; crossing owners clones slice headers
// only, and any mutation must replace a whole element or rebuild the payload.
...
func buildOpenAIWSReplayInputSequenceFromItems(
	...
	return combineOpenAIWSReplayItems(previousFullInput, currentItems), true
}
...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	recoverIngressPrevResponseNotFound := func(relayErr error, turn int, connID string) bool {
		...
		// function_call_output requests must keep previous_response_id because
		// upstream needs the response chain to match tool outputs to prior tool calls.
	...
	for {
		...
		replayHasFunctionCallOutput := currentTurnReplayInputExists && openAIWSRawItemsHasFunctionCallOutput(currentTurnReplayInput)
