...
func (s *ScheduledTestRunnerService) runOnePlan(ctx context.Context, plan *ScheduledTestPlan) {
	...
	if result.Status == "success" {
		s.tryRecoverAccount(ctx, plan.AccountID, plan.ID, plan.ModelID, plan.AutoRecover)
	}
...
func (s *ScheduledTestRunnerService) tryRecoverAccount(ctx context.Context, accountID int64, planID int64, testedModel string, recoverError bool) {
	...
	recovery, err := s.rateLimitSvc.RecoverAccountState(ctx, accountID, AccountRecoveryOptions{
		...
		SuccessfulTestModel: testedModel,
	})
	if err != nil {
