...
func TestHandleSmartRetry_503_LongDelay_NoSingleAccountRetry_StillSwitches(t *testing.T) {
	...
	// 对照：多账号模式应设模型限流；Gemini 同时写具体模型和 capability 级避让 key。
	requireModelRateLimitKeys(t, repo.modelRateLimitCalls, "gemini-3-pro-high", antigravityGeminiModelRateLimitKey)
}
