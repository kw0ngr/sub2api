...
type OpenAIGatewayService struct {
	...
	openaiCodexTurnStateMu      sync.Mutex
	openaiCodexTurnStateOrigins map[string]openAICodexTurnStateOrigin
	openaiCodexTurnStateWrites  uint64
...
func (s *OpenAIGatewayService) getUpstreamFaultMapper() *UpstreamFaultMapper {
	if s != nil && s.upstreamFaultMapper != nil {
		return s.upstreamFaultMapper
	}
...
func (s *OpenAIGatewayService) SelectAccountForModelWithExclusions(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*Account, error) {
	return s.selectAccountForModelWithExclusionsForPlatform(ctx, PlatformOpenAI, groupID, sessionHash, requestedModel, excludedIDs, 0)
}

func (s *OpenAIGatewayService) selectAccountForModelWithExclusionsForPlatform(ctx context.Context, platform string, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, stickyAccountID int64) (*Account, error) {
	...
	if account := s.tryStickySessionHitForPlatform(ctx, platform, groupID, sessionHash, requestedModel, excludedIDs, stickyAccountID); account != nil {
		return account, nil
	}
	...
	selected := s.selectBestAccountForPlatform(ctx, platform, groupID, accounts, requestedModel, excludedIDs)
...
func (s *OpenAIGatewayService) tryStickySessionHitForPlatform(ctx context.Context, platform string, groupID *int64, sessionHash, requestedModel string, excludedIDs map[int64]struct{}, stickyAccountID int64) *Account {
	platform = normalizeOpenAICompatiblePlatform(platform)
	if sessionHash == "" {
		return nil
	}
...
func (s *OpenAIGatewayService) selectBestAccountForPlatform(ctx context.Context, platform string, groupID *int64, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}) *Account {
	platform = normalizeOpenAICompatiblePlatform(platform)
	...
	for i := range accounts {
		...
		fresh := s.resolveFreshSchedulableOpenAIAccountForPlatform(ctx, acc, requestedModel, platform)
		if fresh == nil {
			continue
		}
		fresh = s.recheckSelectedOpenAIAccountFromDBForPlatform(ctx, fresh, requestedModel, platform)
		if fresh == nil {
			continue
		}
...
func (s *OpenAIGatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*AccountSelectionResult, error) {
	return s.SelectAccountWithLoadAwarenessForPlatform(ctx, PlatformOpenAI, groupID, sessionHash, requestedModel, excludedIDs)
}

// SelectAccountWithLoadAwarenessForPlatform selects an OpenAI-compatible account for the requested provider.
func (s *OpenAIGatewayService) SelectAccountWithLoadAwarenessForPlatform(ctx context.Context, platform string, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*AccountSelectionResult, error) {
	platform = normalizeOpenAICompatiblePlatform(platform)
	if s.checkChannelPricingRestriction(ctx, groupID, requestedModel) {
		slog.Warn("channel pricing restriction blocked request",
			"group_id", derefGroupID(groupID),
	...
	if s.concurrencyService == nil || !cfg.LoadBatchEnabled {
		account, err := s.selectAccountForModelWithExclusionsForPlatform(ctx, platform, groupID, sessionHash, requestedModel, excludedIDs, stickyAccountID)
		if err != nil {
			return nil, err
		}
	...
	if err != nil {
		...
		if len(available) > 0 {
			...
			for _, item := range available {
				fresh := s.resolveFreshSchedulableOpenAIAccountForPlatform(ctx, item.account, requestedModel, platform)
				if fresh == nil {
					continue
				}
				if needsUpstreamCheck && s.isUpstreamModelRestrictedByChannel(ctx, *groupID, fresh, requestedModel) {
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDB(ctx context.Context, account *Account, requestedModel string) *Account {
	return s.recheckSelectedOpenAIAccountFromDBForPlatform(ctx, account, requestedModel, PlatformOpenAI)
}

func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDBForPlatform(ctx context.Context, account *Account, requestedModel string, platform string) *Account {
	platform = normalizeOpenAICompatiblePlatform(platform)
	if account == nil {
		return nil
	}
...
func isSchedulableOpenAICompatibleAccountForPlatform(account *Account, platform string) bool {
...
func (s *OpenAIGatewayService) shouldFailoverOpenAIUpstreamResponseForAccount(
	...
	// grok-build-0.1 is an optional CLI capability probe. Even statuses such as
	// 400/404/426, which are normally returned directly to the caller, must stay
	// request-local and release the scheduler to try another account. A genuine
	// credential rejection is still persisted by HandleUpstreamError.
	if isIgnorableGrokBuildProbeError(account, statusCode, upstreamBody, requestedModel) {
		return true
	}
...
func (s *OpenAIGatewayService) handleFailoverSideEffects(ctx context.Context, resp *http.Response, account *Account, requestedModel ...string) {
	...
	if isOpenAIImageRateLimitError(resp.StatusCode, body) {
		_ = s.rateLimitService.HandleOpenAIImageRateLimit(ctx, account, resp.StatusCode, resp.Header, body)
		return
	}
...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	// Grok 保留 xAI 原生的 instructions 缺省语义，不注入 Codex 默认指令。
	if account.Platform != PlatformGrok && isInstructionsEmpty(reqBody) && !compatMessagesBridge {
		reqBody["instructions"] = "You are a helpful coding assistant."
		bodyModified = true
		markPatchSet("instructions", "You are a helpful coding assistant.")
	}

	if account.Platform != PlatformGrok && isCodexCLI && ensureOpenAIResponsesImageGenerationTool(reqBody) {
		bodyModified = true
		disablePatch()
		logger.LegacyPrintf("service.openai_gateway", "[OpenAI] Injected /responses image_generation tool for Codex client")
	}

	if account.Platform != PlatformGrok && normalizeOpenAIResponsesImageGenerationTools(reqBody) {
		bodyModified = true
		disablePatch()
		logger.LegacyPrintf("service.openai_gateway", "[OpenAI] Normalized /responses image_generation tool payload")
	}
	if account.Platform != PlatformGrok && isCodexCLI && applyCodexImageGenerationBridgeInstructions(reqBody) {
		bodyModified = true
		disablePatch()
	...
	// - 其他模型（如 gpt-5.5）将 max 映射为 xhigh
	{
		...
		if account.Platform == PlatformOpenAI && stripOpenAIResponsesReasoningUnsupportedFields(reqBody, modelForEffort) {
			bodyModified = true
			disablePatch()
		}
	}
	...
	if !isCodexCLI {
		if maxOutputTokens, hasMaxOutputTokens := reqBody["max_output_tokens"]; hasMaxOutputTokens {
			...
			case PlatformGrok:
				// xAI Responses 原生支持 max_output_tokens，保持客户端值。
	...
	for {
		...
		serviceTier := observedUpstreamServiceTier(c)
		if serviceTier == nil {
...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	if !isOpenAIResponsesCompactPath(c) {
		...
		fpIDs := resolveCodexFingerprintIDsFromRequest(account, clientHeaders)
		if fpBody, fpChanged, fpErr := applyCodexFingerprintClientMetadataRaw(body, fpIDs); fpErr != nil {
			return nil, fpErr
		} else if fpChanged {
			body = fpBody
		}
	...
	SetOpsLatencyMs(c, OpsUpstreamLatencyMsKey, time.Since(upstreamStart).Milliseconds())
	if err != nil {
		return nil, s.handleOpenAIUpstreamTransportError(ctx, c, account, err, true, policyModel)
	}
	defer func() { _ = resp.Body.Close() }()
...
func (s *OpenAIGatewayService) handleFailoverErrorResponsePassthrough(
	...
	if account != nil && account.Platform == PlatformOpenAI && isOpenAIContentPolicyRejection(resp.StatusCode, body) {
		appendOpsUpstreamError(c, OpsUpstreamErrorEvent{
			Platform:             account.Platform,
			AccountID:            account.ID,
			AccountName:          account.Name,
		...
		if upstreamMsg == "" {
			return fmt.Errorf("upstream content policy rejection: %d", resp.StatusCode)
		}
		return fmt.Errorf("upstream content policy rejection: %d message=%s", resp.StatusCode, upstreamMsg)
	}
...
func openAIStreamItemHasVisibleOutput(item gjson.Result) bool {
	if item.Get("arguments").String() != "" || item.Get("input").String() != "" || item.Get("result").String() != "" || item.Get("name").String() != "" || item.Get("function.name").String() != "" || item.Get("tool_calls.0.function.name").String() != "" {
		return true
	}
...
func openAIStreamDataStartsVisibleOutput(data, eventType string) bool {
	...
	case "response.completed", "response.done":
		for _, item := range openAIResponseOutputItems([]byte(trimmed)) {
...
func openAISSEHasVisibleOutputBeforeTerminal(body string) bool {
...
func openAIResponseUsageIsPresent(data []byte) bool {
	_, ok := extractOpenAIUsageFromJSONBytes(data)
	return ok
}

func openAIResponseOutputItems(data []byte) []gjson.Result {
...
func openAIResponsesCompletedEventIsEmpty(data []byte, usageSeen bool) bool {
	if len(data) == 0 || !gjson.ValidBytes(data) {
		return false
	}
	if usageSeen || openAIResponseUsageIsPresent(data) {
		return false
	}
	if gjson.GetBytes(data, "error").Exists() || gjson.GetBytes(data, "response.error").Exists() {
		return false
	}
	for _, item := range openAIResponseOutputItems(data) {
		if openAIStreamItemHasVisibleOutput(item) {
			return false
		}
	}
...
func openAIStreamFailedEventShouldFailover(payload []byte, message string) bool {
	...
	if isOpenAITransientProcessingError(http.StatusBadRequest, message, payload) || hasOpenAIServerOverloadedCode(payload) {
		return true
	}
...
func openAIStreamErrorEventShouldFailover(payload []byte, message string) bool {
	...
	if openAIStreamFailedEventHTTPStatus(payload, message) == http.StatusTooManyRequests {
		return true
	}
	...
	if isOpenAITransientProcessingError(http.StatusBadRequest, msg, payload) || hasOpenAIServerOverloadedCode(payload) {
		return true
	}
	return containsAny(combined, "temporarily", "temporary", "try again", "timeout", "timed out", "rate_limit", "rate limit", "capacity", "overloaded", "unavailable")
}
...
func (s *OpenAIGatewayService) newOpenAIStreamFailoverError(
	...
	if s != nil && s.rateLimitService != nil && account != nil && len(payload) > 0 {
		...
		_ = s.rateLimitService.HandleUpstreamError(
			ctx, account, statusCode, headers, body, openAIModelForUpstreamError(nil, requestedModel...),
		)
	}
	return &UpstreamFailoverError{
...
func (s *OpenAIGatewayService) newOpenAIResponsesEmptyCompletedFailoverError(c *gin.Context, account *Account, passthrough bool, resp *http.Response, payload []byte) *UpstreamFailoverError {
	upstreamRequestID := ""
	if resp != nil {
		upstreamRequestID = strings.TrimSpace(resp.Header.Get("x-request-id"))
	}
	return s.newOpenAIStreamFailoverError(c, account, passthrough, upstreamRequestID, payload, openAIResponsesEmptyCompletedMessage)
}
...
func (s *OpenAIGatewayService) nonStreamingTerminalFailureFailover(c *gin.Context, resp *http.Response, account *Account, passthrough bool, terminalType string, payload []byte, message string, requestedModel ...string) *UpstreamFailoverError {
	...
	return s.newOpenAIStreamFailoverError(c, account, passthrough, upstreamRequestID, payload, message, requestedModel...)
}
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	usableOutputSeen := false
	validUsageSeen := false
	failedMessage := ""
	...
	for scanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if (eventType == "response.completed" || eventType == "response.done") && !sawFailedEvent && !usableOutputSeen && !clientOutputStarted && openAIResponsesCompletedEventIsEmpty(dataBytes, validUsageSeen) {
				return &openaiStreamingResultPassthrough{usage: usage, firstTokenMs: firstTokenMs}, s.newOpenAIResponsesEmptyCompletedFailoverError(c, account, true, resp, dataBytes)
			}
...
func (s *OpenAIGatewayService) handlePassthroughSSEToJSON(resp *http.Response, c *gin.Context, account *Account, body []byte, requestedModel ...string) (*OpenAIUsage, error) {
	...
	visibleOutputSeen := openAISSEHasVisibleOutputBeforeTerminal(bodyText)

	usage := &OpenAIUsage{}
	if ok {
		if terminalOK && (terminalType == "response.completed" || terminalType == "response.done") && !visibleOutputSeen && openAIResponsesCompletedEventIsEmpty(terminalPayload, false) {
			if account != nil {
				return nil, s.newOpenAIResponsesEmptyCompletedFailoverError(c, account, true, resp, terminalPayload)
			}
			return nil, s.writeOpenAINonStreamingProtocolError(resp, c, openAIResponsesEmptyCompletedMessage)
		}
...
func isOpenAIInternalOAuthAccount(account *Account) bool {
	return account != nil && account.Type == AccountTypeOAuth &&
		(account.Platform == "" || account.Platform == PlatformOpenAI)
}
...
func (s *OpenAIGatewayService) handleErrorResponse(
	...
	mapped := s.getUpstreamFaultMapper().Map(
		resp.StatusCode,
	...
	MarkResponseCommitted(c)
	c.JSON(mapped.StatusCode, gin.H{
		"error": gin.H{
...
func (s *OpenAIGatewayService) handleCompatErrorResponse(
	...
	MarkResponseCommitted(c)
	writeError(c, mapped.StatusCode, mapped.ErrorType, mapped.Message)
...
func (s *OpenAIGatewayService) handleStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel string) (*openaiStreamingResult, error) {
	...
	// 记录上次实际写入下游的时间。pre-output failover 会持续读取并缓冲
	// response.created / response.in_progress，因此 keepalive 不能依赖上游读取时间。
	...
	handleScanErr := func(scanErr error) (*openaiStreamingResult, error, bool) {
		...
		if !openAIStreamClientOutputStarted(c, clientOutputStarted) {
			...
			return resultWithUsage(), s.newOpenAIStreamFailoverError(c, account, false, upstreamRequestID, nil, msg, mappedModel), true
		}
	...
	processSSELine := func(line string, queueDrained bool) {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if eventType == "error" && !openAIStreamClientOutputStarted(c, clientOutputStarted) {
				errorMessage := extractOpenAISSEErrorMessage(dataBytes)
				if openAIStreamErrorEventShouldFailover(dataBytes, errorMessage) {
					streamFailoverErr = s.newOpenAIStreamFailoverError(c, account, false, upstreamRequestID, dataBytes, errorMessage, mappedModel)
					return
				}
			}
			if eventType == "response.failed" {
				...
				if !openAIStreamClientOutputStarted(c, clientOutputStarted) {
					...
					if openAIStreamFailedEventShouldFailover(dataBytes, failedMessage) {
						sawFailedEvent = true
						streamFailoverErr = s.newOpenAIStreamFailoverError(c, account, false, upstreamRequestID, dataBytes, failedMessage, mappedModel)
						return
					}
				}
				s.recordOpenAIStreamFailedSideEffects(ctx, account, upstreamRequestID, dataBytes, failedMessage, mappedModel)
			...
			// Codex tool aliases are client compatibility behavior; xAI tool names are native.
			if account == nil || account.Platform != PlatformGrok {
			...
			if account != nil && account.Platform == PlatformOpenAI && (eventType == "response.completed" || eventType == "response.done") && !sawFailedEvent && !usableOutputSeen && !clientOutputStarted && openAIResponsesCompletedEventIsEmpty(dataBytes, validUsageSeen) {
				streamFailoverErr = s.newOpenAIResponsesEmptyCompletedFailoverError(c, account, false, resp, dataBytes)
				return
			}
...
func (s *OpenAIGatewayService) handleNonStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, originalModel, mappedModel string) (*OpenAIUsage, error) {
	...
	if strings.EqualFold(strings.TrimSpace(gjson.GetBytes(body, "status").String()), "failed") ||
		...
		return nil, s.newOpenAIStreamFailoverError(
			c, account, false, strings.TrimSpace(resp.Header.Get("x-request-id")), body, extractOpenAISSEErrorMessage(body), mappedModel,
		)
	}
...
func (s *OpenAIGatewayService) handleSSEToJSON(resp *http.Response, c *gin.Context, account *Account, body []byte, originalModel, mappedModel string) (*OpenAIUsage, error) {
	bodyText := string(body)
	observeOpenAISSEBody(ensureUpstreamResponseModelObservation(c), bodyText)
	finalResponse, ok := extractCodexFinalResponse(bodyText)
	terminalType, terminalPayload, terminalOK := extractOpenAISSETerminalEvent(bodyText)
	...
	if ok {
		if terminalOK && (terminalType == "response.completed" || terminalType == "response.done") && !visibleOutputSeen && openAIResponsesCompletedEventIsEmpty(terminalPayload, false) {
			if account != nil {
				return nil, s.newOpenAIResponsesEmptyCompletedFailoverError(c, account, false, resp, terminalPayload)
			}
			return nil, s.writeOpenAINonStreamingProtocolError(resp, c, openAIResponsesEmptyCompletedMessage)
		}
		if parsedUsage, parsed := extractOpenAIUsageFromJSONBytes(finalResponse); parsed {
		...
		// Codex tool aliases must not rewrite native xAI tool names.
		if account == nil || account.Platform != PlatformGrok {
			body = s.correctToolCallsInResponseBody(body)
		}
	} else {
...
func buildOpenAICompatibleChatCompletionsURL(platform, base string) string {
	...
	if normalized == "" {
		normalized = DefaultAPIKeyBaseURL(platform)
	}
...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	if responseModel := responseModelBillingDeclaration(
		...
		responseModel = normalizedResponseBillingModel(responseModel)
...
func normalizeOpenAIResponsesReasoningCompatibilityBody(body []byte, model string) ([]byte, bool, error) {
	normalized, changed, err := normalizeOpenAIResponsesNestedReasoningEffort(body, model)
	if err != nil {
		return body, false, err
	}
	normalized, aliasChanged, err := normalizeOpenAIResponsesReasoningEffortAlias(normalized, model)
	if err != nil {
		return body, false, err
	}
	stripped, strippedChanged, err := stripOpenAIResponsesReasoningUnsupportedFieldsBytes(normalized, model)
	if err != nil {
		return body, false, err
	}
	return stripped, changed || aliasChanged || strippedChanged, nil
}

func normalizeOpenAIResponsesNestedReasoningEffort(body []byte, model string) ([]byte, bool, error) {
	if len(body) == 0 || !gjson.GetBytes(body, "reasoning.effort").Exists() {
		return body, false, nil
	}
	...
	if err != nil {
		return body, false, fmt.Errorf("normalize responses reasoning.effort: %w", err)
	}
	return normalized, true, nil
}

func stripOpenAIResponsesReasoningUnsupportedFieldsBytes(body []byte, model string) ([]byte, bool, error) {
	if len(body) == 0 || !gjson.ValidBytes(body) {
		return body, false, nil
	}
	var reqBody map[string]any
	...
	if !stripOpenAIResponsesReasoningUnsupportedFields(reqBody, model) {
		return body, false, nil
	}
	normalized, err := json.Marshal(reqBody)
	if err != nil {
		return body, false, fmt.Errorf("serialize reasoning-compatible responses body: %w", err)
	}
	return normalized, true, nil
}

var openAIResponsesReasoningUnsupportedFields = []string{"temperature", "top_p", "top_logprobs", "logprobs"}

func stripOpenAIResponsesReasoningUnsupportedFields(reqBody map[string]any, model string) bool {
	...
	return filterOpenAIResponsesLogprobsInclude(reqBody) || modified
}

func filterOpenAIResponsesLogprobsInclude(reqBody map[string]any) bool {
	include, ok := reqBody["include"]
	if !ok {
		return false
	}
	filter := func(values []any) ([]any, bool) {
		filtered := make([]any, 0, len(values))
		changed := false
		for _, value := range values {
			if s, ok := value.(string); ok && strings.TrimSpace(s) == "message.output_text.logprobs" {
				changed = true
				continue
			}
	...
	case []any:
		filtered, changed := filter(values)
		if changed {
	...
	case []string:
		...
		filtered, changed := filter(anyValues)
		if changed {
			reqBody["include"] = filtered
		}
		return changed
...
func extractEffectiveOpenAIReasoningEffortFromBody(account *Account, body []byte, modelCandidates ...string) *string {
	...
	mapped := normalizeGLMOpenAIReasoningEffortForModel(raw, model)
	if mapped == "" {
		return nil
	}
	return &mapped
...
func (v *openAIRequestView) HasPatches() bool {
	return v != nil && !v.patchesDisabled && len(v.patches) > 0
}
