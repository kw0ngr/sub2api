<template>
  ...
  >
    ...
    >
      ...
      <!-- Anthropic API Key Claude Code 伪装开关 -->
      <div
        v-if="account?.platform === 'anthropic' && account?.type === 'apikey'"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between gap-4">
          ...
          <button
            type="button"
            @click="anthropicClaudeCodeMimicEnabled = !anthropicClaudeCodeMimicEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              anthropicClaudeCodeMimicEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                anthropicClaudeCodeMimicEnabled ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
...
// Load global feature states once. Tests may mock a partial settings API.
const webSearchConfigPromise = adminAPI.settings?.getWebSearchEmulationConfig?.()
if (webSearchConfigPromise) {
  webSearchConfigPromise.then(cfg => {
...
const defaultBaseUrl = computed(() => {
  ...
  if (props.account?.platform === 'glm') {
    return glmCompatMode.value === 'openai' ? GLM_OPENAI_BASE_URL : GLM_ANTHROPIC_BASE_URL
  }
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    glmCompatMode.value = newAccount.platform === 'glm' && credentials.compat_mode === 'openai' ? 'openai' : 'anthropic'
...
watch(glmCompatMode, (newMode, oldMode) => {
  if (props.account?.platform !== 'glm') return
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.type === 'apikey') {
      ...
      if (props.account.platform === 'glm') {
        newCredentials.compat_mode = glmCompatMode.value
      }
