...
/**
 * Sync live supported models for a saved account.
 * @param id - Account ID
...
/**
 * Preview upstream models without a saved account.
...
export async function startAPIKeysHealthCheck(accountIds?: number[]): Promise<APIKeyHealthCheckStartResponse> {
  const payload = accountIds && accountIds.length > 0 ? { account_ids: accountIds } : {}
  const { data } = await apiClient.post<APIKeyHealthCheckStartResponse>('/admin/accounts/apikey-health-check', payload, {
    timeout: 120000
  })
  return data
}
